...
static int tst_parameter_generator()
{
  ...
  if( int err = generator->gen(0, 0, 4) )
    return err;
...
static int tst_composition_toplevel()
{
  ...
  if( int err = g_composition->startup(0, 0, beats) )
    return err;

  if(int err = output->outputCompositionChain("../../algomusicgen/src/comp.mid", g_composition->chains(), beats, g_composition->tempo()))
    return err;
